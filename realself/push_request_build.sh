...
# Build script for RealSelf Consumer Mobile Push Requests
#
# Created by John Lyon-Smith, 2016.
#

PRODUCT_NAME=RealSelf
GIT_REPO_USER=RealSelf
...
# Test
# e.g.
if ! $XCODEBUILD -workspace RealSelf.xcworkspace -scheme "$SCHEME" -sdk iphonesimulator -destination "$SIMULATOR_CONFIG" test; then
 echo ERROR: Tests failed
 exit 1
fi

# Build Archive
if ! $XCODEBUILD -workspace RealSelf.xcworkspace -scheme "$SCHEME" archive; then
